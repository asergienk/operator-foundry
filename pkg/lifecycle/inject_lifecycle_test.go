...
func TestInjectLifecycle_BuilderStagePattern_TracedToContext(t *testing.T) {
	// The catalog is copied into a builder stage that transforms it with a RUN
	// command, and the final stage copies from the builder. The inject step must
	// trace the --from=builder COPY back to the build-context source and write
	// lifecycle.json there.
	//
	// The builder stage has broad COPYs (Makefile, vendor) whose destination
	// shares a prefix with the catalog COPY destination. The most specific
	// (deepest) dest match must be selected, not the first.
	base := t.TempDir()
	lifecycleDir := t.TempDir()
	lifecycleData := []byte(`{"schema":"io.openshift.operators.lifecycles.v1alpha1"}`)

	pkgDir := filepath.Join(base, ".konflux", "catalog", "my-operator")
	if err := os.MkdirAll(pkgDir, 0755); err != nil {
		t.Fatalf("failed to create package dir: %v", err)
	}
